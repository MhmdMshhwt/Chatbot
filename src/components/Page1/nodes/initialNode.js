...
const InitialNode = ({ data }) => {
    ...
    const renderButtons = data.nextChoices.map((item, index) => (
        <Box key={index} sx={{position: 'relative'}}>
            ...
            <Handle type="source" target="node-2" position={Position.Right} id={`handle-${index}`} style={{position: 'absolute', right: '-16px'}}/>
        </Box>
    ));
    ...
    return (
        ...
        >
            ...
            <Box sx={{ background: 'rgba(222, 227, 230, .42)', p: 2, borderTop: '1px dashed rgba(4, 43, 61, .42)', marginTop: '5px', height: 'auto', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                {renderButtons}
            </Box>
